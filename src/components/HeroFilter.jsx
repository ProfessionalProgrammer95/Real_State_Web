...
function HeroFilter({ onResults }) {
  ...
  useEffect(() => {
    if (allProperties.length && !allProperties.some((data) => data.country === selectedCountry)) {
      setSelectedCountry(allProperties[0].country);
    }
  }, [allProperties]);
  ...
  return (
    <section className="max-w-full px-5 sm:px-6">
        ...
        <div className="absolute inset-0 flex flex-col items-center text-white">
          ...
          <div className="mt-4 w-[83%] max-w-[1180px] md:hidden flex items-center justify-center gap-3">
            <button
              ...
              className="rounded-full px-4 py-2 text-sm font-medium bg-white/90 text-[var(--clr-text)] shadow border"
              style={{ borderColor: "var(--clr-border)" }}
            >
          ...
          <div
            className={`mt-60 w-[83%] max-w-[1180px] flex items-center hf-collapsible md:h-auto search-bar ${isMobileSearchOpen ? "open" : ""}`}
          >
      ...
      <div className="relative -mt-10 flex justify-center">
        <div
          className={`w-full max-w-[80%] rounded-full bg-white shadow-[0_18px_36px_rgba(0,0,0,0.08)] px-12 py-8 flex flex-wrap items-center gap-3 justify-between hf-collapsible  ${isMobileFiltersOpen ? "open" : ""}`}
        >
          <div
